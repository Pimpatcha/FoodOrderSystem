...
public interface OrderCustomerRepository extends JpaRepository<OrderCustomer, Integer> {
    ...
    @Transactional
    @Modifying
    @Query("update OrderCustomer o set o.dateOrder = ?1  where o.status = ?2")
    public void updateDatelistOrder(Date date, Status status2);

    @Transactional
    @Modifying
    @Query("update OrderCustomer o set o.timeOrder  = ?1  where o.status = ?2")
    public void updateTimelistOrder(Time time, Status status2);
